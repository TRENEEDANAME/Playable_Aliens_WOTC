...
static function array<X2DataTemplate> CreateTemplates()
{
	...
	Templates.AddItem(CreatePA_SwitchToRobotAbility());
	Templates.AddItem(PurePassive('PA_RobotBattlesuit', "img:///UILibrary_PerkIcons.UIPerk_andromedon_robotbattlesuit"));
...
static function X2AbilityTemplate CreatePA_SwitchToRobotAbility()
{
	local X2AbilityTemplate Template;
	local X2AbilityTrigger_EventListener EventListener;
	local X2Condition_UnitValue UnitValue;
	local X2Effect_SetUnitValue SetUnitValEffect;
	local X2Effect_SwitchToRobot PA_SwitchToRobotEffect;

	`CREATE_X2ABILITY_TEMPLATE(Template, 'PA_SwitchToRobot');
	...
	PA_SwitchToRobotEffect = new class'X2Effect_SwitchToRobot';
	PA_SwitchToRobotEffect.BuildPersistentEffect(1);
	Template.AddTargetEffect(PA_SwitchToRobotEffect);
	...
	Template.BuildNewGameStateFn = TypicalAbility_BuildGameState;
	Template.BuildVisualizationFn = PA_SwitchToRobot_BuildVisualization;
	Template.MergeVisualizationFn = PA_SwitchToRobot_VisualizationMerge;
...
simulated function PA_SwitchToRobot_BuildVisualization(XComGameState VisualizeGameState)
{
	local XComGameStateContext_Ability Context;
	local XComGameStateHistory History;
	...
	PA_SwitchToRobotEffect = X2Effect_SwitchToRobot(Context.ResultContext.TargetEffectResults.Effects[0]);

	if( PA_SwitchToRobotEffect == none )
	{
		...
		PA_SwitchToRobotEffect = X2Effect_SwitchToRobot(AbilityTemplate.AbilityTargetEffects[0]);
	}

	if( PA_SwitchToRobotEffect == none )
	{
		`RedScreenOnce("PA_SwitchToRobot_BuildVisualization: Missing X2Effect_SwitchToRobot -dslonneger @gameplay");
	}
	else
	{
		PA_SwitchToRobotEffect.AddSpawnVisualizationsToTracks(Context, SpawnedUnit, SpawnedUnitTrack, DeadUnit, DeadUnitTrack);
	}
}

static function PA_SwitchToRobot_VisualizationMerge(X2Action BuildTree, out X2Action VisualizationTree)
{
...
static function X2AbilityTemplate CreatePA_ImmunitiesAbility()
{
	local X2AbilityTemplate Template;
	local X2AbilityTrigger_UnitPostBeginPlay Trigger;
	local X2Effect_DamageImmunity DamageImmunity;
