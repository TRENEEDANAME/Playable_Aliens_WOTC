class X2EventListener_AlienCapture extends X2EventListener config(PlayableAdvent);
...
static function X2EventListenerTemplate AddAdventCapture()
{
	...
	`CREATE_X2TEMPLATE(class'X2EventListenerTemplate', Template, 'AlienCaptureEvent');
...
static function X2EventListenerTemplate OverrideEC()
{
	...
	`CREATE_X2TEMPLATE(class'X2EventListenerTemplate', Template, 'RMAlienOverride');
	Template.RegisterInTactical = true;
...
static protected function EventListenerReturn ShouldOverride(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{
	...
	if (KilledUnit.IsSoldier() || !KilledUnit.IsAlien()) 
		return ELR_NoInterrupt;
...
static protected function EventListenerReturn WasAdvent(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{
	...
	// ignore everybody that leaves the field that isn't advent
	if (KilledUnit.IsSoldier() || !KilledUnit.IsAlien()) //added MOCX exclusion
		return ELR_NoInterrupt;

	if(NotApplicableUnit(KilledUnit))
		return ELR_NoInterrupt;
...
static function GiveSoldierToXCOM(XComGameState NewGameState, int ForceLevel, XComGameState_Unit RetrievedUnit, out XComGameState_Unit ReferenceState)
{
	...
	if(RetrievedUnit.GetMyTemplate().CharacterGroupName == 'Sectoid')
		CharacterTemplate = CharTemplateMgr.FindCharacterTemplate('PA_Sectoid');

	if(RetrievedUnit.GetMyTemplate().CharacterGroupName == 'Muton')
		CharacterTemplate = CharTemplateMgr.FindCharacterTemplate('PA_Muton');
		
	if(RetrievedUnit.GetMyTemplate().CharacterGroupName == 'Viper')
		CharacterTemplate = CharTemplateMgr.FindCharacterTemplate('PA_Viper2');

	if(RetrievedUnit.GetMyTemplate().CharacterGroupName == 'Andromedon')
		CharacterTemplate = CharTemplateMgr.FindCharacterTemplate('PA_Andromedon');

	if(RetrievedUnit.GetMyTemplate().CharacterGroupName == 'Berserker')
		CharacterTemplate = CharTemplateMgr.FindCharacterTemplate('PA_Berserker');

	if(RetrievedUnit.GetMyTemplate().CharacterGroupName == 'Archon')
		CharacterTemplate = CharTemplateMgr.FindCharacterTemplate('PA_Archon');
...
static function bool NotApplicableUnit(XComGameState_Unit UnitState)
{
	if(UnitState.GetMyTemplate().CharacterGroupName == 'Sectoid')
		return false;

	if(UnitState.GetMyTemplate().CharacterGroupName == 'Muton')
		return false;
		
	if(UnitState.GetMyTemplate().CharacterGroupName == 'Viper')
		return false;

	if(UnitState.GetMyTemplate().CharacterGroupName == 'Andromedon')
		return false;
